...
type symbol =
	...
	fct = (lit,opr,lod,sto,cal,int,jmp,jpc,red,wrt); {funcions}
	...
	RED l,a:  从键盘读取数据存到层差为l的层，相对地址为a的变量
	WRT l,a:  取层差为l的层，相对地址为a的变量,打印到屏幕中}
					...
					if sym = lparen then
						...
						if sym = ident then 
							begin i := position(id);
								...
								else begin if table[i].kind <> variable then
										begin {读入到常量} error(12);i :=0 
										end;
									end;
								if i <> 0 then
							...
							end else error(42) {missing para};
					end else error(43) {missing lpa};
				statement(fsys)
			end else
			{write}
					...
					if sym = lparen then
						...
						if sym = ident then 
							begin i := position(id);
								...
								if i <> 0 then
									with table[i] do gen(wrt,lev-level,adr);
								getsym;
								while sym = comma do
								begin 
									getsym;
